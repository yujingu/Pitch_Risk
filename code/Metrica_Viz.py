...
def plot_frame( hometeam, awayteam, figax=None, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
    ...
    # plot ball
    ax.plot( hometeam['ball_x'], hometeam['ball_y'], 'yo', markersize=6, alpha=1.0, linewidth=0)
    return fig,ax
...
def plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68)):
    ...
    #plot_events( events.loc[event_id:event_id], figax = (fig,ax), indicators = ['Marker','Arrow'], annotate=False, color= 'k', alpha=1 ) #2022/6/15コメントアウト　謎のエラー
    ...
    #ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),vmin=0.0,vmax=1.0,cmap=cmap,alpha=0.5)
...
#2021/8/25作成
def plot_pitchcontrol_for_normal(x,  tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68)):
    #plot_pitchcontrol_for_eventを参考に作成
    ...
    # plot frame and event
    fig,ax = plot_pitch(field_color='white', field_dimen = field_dimen)
    plot_frame( tracking_home.loc[x,:], tracking_away.loc[x,:], figax=(fig,ax), PlayerAlpha=alpha, include_player_velocities=include_player_velocities, annotate=annotate )

    # plot pitch control surface
    #ひとまずホームを攻撃チームという体で作成
    cmap = 'bwr'
    ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',vmin=0.0,vmax=1.0,cmap=cmap,alpha=0.5)
...
class draggablecircle:
    ...
    # マウスイベントへの接続
    def connect(self):
        self.cidpress = self.cir.figure.canvas.mpl_connect('button_press_event', self.on_press)
        self.cidrelease = self.cir.figure.canvas.mpl_connect('button_release_event', self.on_release)
        self.cidmotion = self.cir.figure.canvas.mpl_connect('motion_notify_event', self.on_motion)
    ...
    def on_press(self,event):
        if event.inaxes!=self.cir.axes:
            return
    
        contains,attrd=self.cir.contains(event)
        if not contains:
            return
    
        print('event contains',self.cir.xy)
        x0,y0=self.cir.xy
    ...
    def on_motion(self,event):
        ...
        self.cir.set_x(x0+dx)
        self.cir.set_y(y0+dy)
    
        self.cir.figure.canvas.draw()
        #PPCF,tr_h,tr_a = mpc.generate_pitch_control_for_drag(n, x0, y0, dx, dy, tracking_h, tracking_a, parameters, [mio.find_goalkeeper(tracking_h),mio.find_goalkeeper(tracking_a)], field_dimen = (106.,68.,), n_grid_cells_x = 50)
        #plot_pitchcontrol_for_drag(n, tr_h, tr_a, PPCF, annotate=True )
...
def plot_pitchcontrol_for_drag(params, x, tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68)):
    #plot_pitchcontrol_for_eventを参考に作成
    ...
    # plot frame and event
    fig,ax = plot_pitch(field_color='white', field_dimen = field_dimen)
    plot_frame_draggable(params, x, tracking_home, tracking_away, figax=(fig,ax), PlayerAlpha=alpha, include_player_velocities=include_player_velocities, annotate=annotate )
...
tracking_a=[]
parameters=[]
def plot_frame_draggable(params, x, tracking_home, tracking_away, figax=None, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False, ):
    hometeam=tracking_home.loc[x,:]
    awayteam=tracking_away.loc[x,:]
    if figax is None: # create new pitch 
        fig,ax = plot_pitch( field_dimen = field_dimen )
    else: # overlay on a previously generated pitch
        fig,ax = figax # unpack tuple
    ...
    for team,color in zip( [hometeam,awayteam], team_colors) :
        x_columns = [c for c in team.keys() if c[-2:].lower()=='_x' and c!='ball_x'] # column header for player x positions
        y_columns = [c for c in team.keys() if c[-2:].lower()=='_y' and c!='ball_y'] # column header for player y positions
        for pos_x,pos_y in zip(x_columns,y_columns):
            cir=Rectangle((team[pos_x]-1,team[pos_y]-1),2,2,fc=color)
            ax.add_patch(cir)
            dr.append(draggablecircle(cir))
            dr[-1].connect()
        """
        if include_player_velocities:
            vx_columns = ['{}_vx'.format(c[:-2]) for c in x_columns] # column header for player x positions
        ...
        if annotate:
            ...
            """
    n=x
    tracking_h=tracking_home
    tracking_a=tracking_away
    parameters=params
    # plot ball
...
#2021/11/18追加
def plot_pitchcontrol_for_d( event_id, events,  tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68)):
    """ plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF )
    ...
    #ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',vmin=0.0,vmax=0.05,cmap=cmap,alpha=0.5)
    #2022/6ver.
    #ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',cmap=cmap,alpha=0.5,vmin=0.0,vmax=0.05)
    #ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',cmap=cmap,alpha=0.5,vmin=0.0,vmax=0.03)#ver.1
    ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',cmap=cmap,alpha=0.5,vmin=0.0,vmax=0.1)#ver.2
    
    return fig,ax


#2021/12/2追加
def plot_pitchcontrol_for_xG( event_id, events,  tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68)):
    """ plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF )
    ...
    if pass_team=='Home':
        ...
        cmap = 'bwr_r'
    ax.imshow(np.flipud(PPCF), extent=(-field_dimen[0]/2., field_dimen[0]/2., -field_dimen[1]/2., field_dimen[1]/2.),interpolation='spline36',vmin=0.0,vmax=0.3,cmap=cmap,alpha=0.5)
