...
#2021/8/25作成
def generate_pitch_control_for_normal(x, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    #generate_pitch_control_for_eventを参考に作成
    ball_start_pos=np.array([tracking_home.loc[x]['ball_x'],tracking_away.loc[x]['ball_y']])
    # break the pitch down into a grid
    n_grid_cells_y = int(n_grid_cells_x*field_dimen[1]/field_dimen[0])
    dx = field_dimen[0]/n_grid_cells_x
    dy = field_dimen[1]/n_grid_cells_y
    ...
    attacking_players = initialise_players(tracking_home.loc[x,:],'Home',params,GK_numbers[0])
    defending_players = initialise_players(tracking_away.loc[x,:],'Away',params,GK_numbers[1])
...
#2021/11/5作成
def generate_pitch_control_for_drag(x, x0,y0,dx0,dy0, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    #generate_pitch_control_for_eventを参考に作成
    ball_start_pos=np.array([tracking_home.loc[x]['ball_x'],tracking_away.loc[x]['ball_y']])
    # break the pitch down into a grid
    n_grid_cells_y = int(n_grid_cells_x*field_dimen[1]/field_dimen[0])
    ...
    PPCFd = np.zeros( shape = (len(ygrid), len(xgrid)) )
    # initialise player positions and velocities for pitch control calc (so that we're not repeating this at each grid cell position)
    for pos_x,pos_y in zip(tracking_home.loc[n,:].filter(like='_x').index,tracking_home.loc[n,:].filter(like='_y').index):
        if tracking_home.loc[n,:][pos_x]==x0:
            tracking_home.loc[n,:][pos_x]=x0+dx0
        if tracking_home.loc[n,:][pos_y]==y0:
            tracking_home.loc[n,:][pos_y]=y0+dy0
    for pos_x,pos_y in zip(tracking_away.loc[n,:].filter(like='_x').index,tracking_away.loc[n,:].filter(like='_y').index):
        if tracking_away.loc[n,:][pos_x]==x0:
            tracking_away.loc[n,:][pos_x]=x0+dx0
        if tracking_away.loc[n,:][pos_y]==y0:
            tracking_away.loc[n,:][pos_y]=y0+dy0
    ...
    checksum = np.sum( PPCFa + PPCFd ) / float(n_grid_cells_y*n_grid_cells_x ) 
    assert 1-checksum < params['model_converge_tol'], "Checksum failed: %1.3f" % (1-checksum)
    return PPCFa,tracking_home,tracking_away